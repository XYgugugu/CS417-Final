...
namespace PVZ3D.Plants
{
    public class WallnutPlant : PlantBase
    {
        ...
        {
            return !isTallnut && upgradeDefinition != null && upgradeDefinition.Archetype == PlantArchetype.Wallnut;
        }
        ...
        private void EnsureTallnutVisuals()
        {
            ...
            if (crownVisual == null)
            {
                crownVisual = CreateUpgradePart("TallnutTop", PrimitiveType.Cylinder, new Vector3(0f, 1.18f, 0f), new Vector3(0.36f, 0.06f, 0.36f), new Color(0.62f, 0.42f, 0.2f), Quaternion.identity);
            }

            if (browVisual == null)
            {
                browVisual = CreateUpgradePart("TallnutBrow", PrimitiveType.Cube, new Vector3(-0.04f, 0.92f, 0.28f), new Vector3(0.3f, 0.05f, 0.06f), new Color(0.2f, 0.1f, 0.04f), Quaternion.Euler(0f, 0f, -8f));
            }
        }
