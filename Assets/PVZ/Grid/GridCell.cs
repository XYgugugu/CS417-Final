...
namespace PVZ3D.Grid
{
    public class GridCell : MonoBehaviour, IPointerClickHandler
    {
        ...
        {
            PlantPlacementManager.Instance?.TryPlaceSelectedAt(this);
        }
        ...
        private void UpdateVisual()
        {
            ...
            Color targetColor = IsOccupied ? occupiedColor : (xrHovering ? hoverColor : emptyColor);
            ...
            for (int i = 0; i < tileRenderers.Length; i++)
            {
                ...
                if (renderer != null)
                {
                    RuntimeVisualMaterialUtility.ApplyColor(renderer, targetColor);
                }
            }
        }
